...
checar("--share-net" in linha_push, "git push recebe a exceção de rede esperada")
...
print("\n=== 6c. gh recebe rede somente para consultas ===")
checar(not recusou(execucao.run_command(
    "gh issue view 246 --repo aws-cloudformation/cloudformation-validate")),
    "gh issue view passa pela revisão estrutural")
for tentativa in [
    "gh issue create --title x --body y",
    "gh issue close 246 --repo aws-cloudformation/cloudformation-validate",
    "gh pr merge 10 --repo dono/repo",
    "gh api --method POST /repos/dono/repo/issues",
    "gh auth token",
    "gh auth status --show-token",
]:
    checar(recusou(execucao.run_command(tentativa, autorizado=True)),
           f"gh mutável/sensível continua recusado: {tentativa!r}")
linha_gh = execucao.montar_bwrap(["gh", "issue", "view"], raiz, rede=True)
checar("--share-net" in linha_gh and "GH_CONFIG_DIR" in linha_gh,
       "gh somente leitura recebe rede e configuração isolada")
checar("SSH_AUTH_SOCK" not in linha_gh,
       "gh não recebe socket SSH desnecessariamente")
