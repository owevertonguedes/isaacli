...
saida = execucao.executar_comando("ls")
...
saida = execucao.executar_comando("wc -l dentro.py outro.py")
checar("3" in saida and "dentro.py" in saida, f"wc -l conta as linhas: {saida!r}")

saida = execucao.executar_comando('python3 -c "print(2+2)"')
...
saida = execucao.executar_comando('python3 -c "import time; print(7*6)"')
checar("42" in saida, f"';' dentro de aspas nao e confundido com shell: {saida!r}")

print("\n=== 2. erro do comando volta CRU, nao mastigado ===")
saida = execucao.executar_comando("cat nao_existe.txt")
...
for tentativa in [
    ...
    saida = execucao.executar_comando(tentativa)
...
checar(not recusou(execucao.executar_comando("git status")), "git status passa")
checar(recusou(execucao.executar_comando("git rebase main")), "git rebase recusado (fora da lista)")
checar(recusou(execucao.executar_comando("git clone http://x")), "git clone recusado (fora da lista)")
for tentativa in ["git push --force", "git push -f", "git push --force-with-lease",
                  "git push origin main --force"]:
    checar(recusou(execucao.executar_comando(tentativa)), f"recusado: {tentativa!r}")
...
saida = execucao.executar_comando("git add a.txt")
checar(not recusou(saida), f"git add liberado: {saida!r}")
saida = execucao.executar_comando("git commit -m msg")
...
saida = execucao.executar_comando("git push origin master")
...
checar(not recusou(execucao.executar_comando("find . -name '*.py'")),
...
saida = execucao.executar_comando("curl http://exemplo.com")
checar("Permitidos:" in saida, "a recusa DIZ o que e permitido, nao so 'nao'")

print("\n=== 6b. graphify so consulta grafo local ===")
checar(not recusou(execucao.executar_comando('graphify query "onde fica X"')),
...
saida = execucao.executar_comando(
...
saida = execucao.executar_comando(f"""python3 -c "print(open('{isca}').read())" """)
checar("se este arquivo" not in saida,
       f"python3 NAO conseguiu nem LER a isca de fora ({saida[:200]!r})")

saida = execucao.executar_comando("""python3 -c "open('/etc/passwd','a').write('x')" """)
...
saida = execucao.executar_comando(f"ls {Path.home() / '.ssh'}")
checar("(código de saída: 0)" not in saida,
       f".ssh do usuario nao esta acessivel dentro do sandbox ({saida[:200]!r})")
saida = execucao.executar_comando(f"ls {Path.home() / 'DevTools'}")
checar("(código de saída: 0)" not in saida,
...
saida = execucao.executar_comando("""python3 -c "open('criado.txt','w').write('ok')" """)
...
saida = execucao.executar_comando("""python3 -c "import time; time.sleep(60)" """)
...
saida = execucao.executar_comando("""python3 -c "print('x'*100000)" """)
