...
para aquela chamada (`--share-net`, ver `_precisa_de_rede`). Consultas `gh`
estritamente somente leitura também recebem rede; mutações seguem bloqueadas.
Todo o resto (python3, pytest, git status/diff/...) continua sem rede nenhuma. HOME
...
# Consultas do GitHub que não alteram estado remoto. `gh api`, auth, issue/pr
# create/edit/close e workflows ficam estruturalmente fora, mesmo após aprovação.
GH_PERMITIDOS = {
...
GH_FLAGS_PROIBIDAS = {"--web", "--show-token"}
...
def revisar(cmd, autorizado=False):
    ...
    if programa == "gh":
        ...
        if rota not in GH_PERMITIDOS:
            permitidos = ", ".join(" ".join(item) for item in sorted(GH_PERMITIDOS))
            raise Recusado(
                "esta operação do gh não é somente leitura. "
                f"Consultas permitidas: {permitidos}")
        flags_proibidas = set(partes) & GH_FLAGS_PROIBIDAS
        if flags_proibidas:
            raise Recusado(
                "flag do gh não permitida nesta consulta: "
                + ", ".join(sorted(flags_proibidas)))

    return partes


def _precisa_de_rede(partes):
    """Rede só para push e consultas gh estritamente revisadas."""
    return (len(partes) >= 2 and partes[0] == "git" and partes[1] == "push") or (
        partes and partes[0] == "gh"
    )
...
def montar_bwrap(argv, raiz, rede=False):
    ...
    rede=True reabre a rede com --share-net para `git push` ou consultas gh já
    revisadas. Push recebe o socket do ssh-agent e known_hosts; gh recebe apenas
    sua configuração em montagem somente leitura. HOME continua sendo a pasta
    de trabalho, sem expor credenciais a `cat`/`python3`.
    """
    ...
    if rede:
        ...
        if argv and argv[0] == "gh" and GH_CONFIG_HOST.is_dir():
            destino = "/tmp/gh-config"
            linha += ["--ro-bind", str(GH_CONFIG_HOST), destino,
                      "--setenv", "GH_CONFIG_DIR", destino,
                      "--setenv", "GH_PAGER", "cat",
                      "--setenv", "PAGER", "cat"]
...
ESQUEMA = {
    ...
    "function": {
        ...
        "description": (
            ...
            "no proprio 'git push' e nas consultas somente leitura do gh. Use pra "
