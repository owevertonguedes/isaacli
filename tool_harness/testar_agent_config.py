...
try:
    ...
    agent.chamar("gpt-oss", mensagens, usar_tools=False, thinking="high", num_ctx=32768)
...
assert capturados[0]["options"]["num_ctx"] == 32768, "contexto selecionado precisa chegar ao Ollama"
...
print("AGENT CONFIG OK — thinking e contexto separados e enviados ao Ollama")
...
# Ollama envia o raciocínio em message.thinking. Ele alimenta apenas o progresso:
# não pode vazar para a resposta visível nem para o histórico.
def urlopen_ollama_stream(req, timeout=0):
    return Resposta(
        b'{"message":{"role":"assistant","thinking":"passo oculto"}}\n'
        b'{"message":{"role":"assistant","content":"resposta"}}\n'
        b'{"done":true,"eval_count":2,"eval_duration":100000000}\n'
    )
original = agent.urllib.request.urlopen
try:
    agent.urllib.request.urlopen = urlopen_ollama_stream
    pensamentos, visiveis = [], []
    msg = agent.chamar_stream(
        "modelo", [{"role": "user", "content": "oi"}], usar_tools=False,
        on_token=visiveis.append, on_thinking=pensamentos.append,
    )
finally:
    agent.urllib.request.urlopen = original
assert pensamentos == ["passo oculto"]
assert visiveis == ["resposta"] and msg["content"] == "resposta"
assert "passo oculto" not in json.dumps(msg, ensure_ascii=False)
print("AGENT THINKING OK — progresso oculto separado da resposta")

# Há modelos Qwen que encerram o turno com toda a resposta em thinking e
# content vazio. Nesse caso específico, o texto precisa virar a resposta visível.
def urlopen_ollama_so_thinking(req, timeout=0):
    return Resposta(
        b'{"message":{"role":"assistant","thinking":"resposta recuperada"}}\n'
        b'{"done":true,"eval_count":2,"eval_duration":100000000}\n'
    )
original = agent.urllib.request.urlopen
try:
    agent.urllib.request.urlopen = urlopen_ollama_so_thinking
    pensamentos, visiveis = [], []
    msg = agent.chamar_stream(
        "modelo", [{"role": "user", "content": "oi"}], usar_tools=False,
        on_token=visiveis.append, on_thinking=pensamentos.append,
    ...
    agent.urllib.request.urlopen = original
assert pensamentos == ["resposta recuperada"]
assert visiveis == ["resposta recuperada"]
assert msg["content"] == "resposta recuperada"
print("AGENT THINKING FALLBACK OK — resposta vazia recupera o texto gerado")

# Resposta vazia sem tool_call é um erro real do modelo/provedor: precisa
# subir como está, sem tentativa oculta com outra ferramenta nem retry
# automático. Foi removido de propósito: mascarava a causa e arriscava loop.
schemas_vistos = []
chamar_original = agent.chamar
try:
    def chamar_vazia(*_args, tools_schema=None, **_kwargs):
        schemas_vistos.append([
            item["function"]["name"] for item in (tools_schema or agent.tools.SCHEMA)
        ])
        return {"role": "assistant", "content": "", "_usage": {"eval_count": 80}}
    agent.chamar = chamar_vazia
    resultado = agent.rodar(
        "crie um markdown DESIGN.md", "modelo-qualquer", verbose=False,
    )
finally:
    agent.chamar = chamar_original
assert len(schemas_vistos) == 1, "resposta vazia não deve gerar segunda tentativa oculta"
assert not resultado["final"], "resposta vazia sobe como está, sem texto inventado"
assert resultado["uso"]["eval_count"] == 80
print("AGENT SEM FALLBACK OCULTO OK — resposta vazia sobe como erro real, sem retry escondido")
