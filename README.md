...
Requires Python 3.10+ and `bwrap` (`bubblewrap`). An engine is not a prerequisite: setup lists all four sources and each line says what this machine already has. [Ollama](https://ollama.com) is used where it is installed, isaacli can install llama.cpp for you and serve a GGUF you already have, and the other two sources are remote.
...
Setup selects the interface language, engine, model, context and reasoning effort. The engine can be a local Ollama model with native tool calling, a GGUF file on this machine served through llama.cpp (which isaacli installs into your own directory if you do not have it), a configurable OpenAI-compatible endpoint, or the guided Kaggle GPU integration. API keys are stored outside the workspace in a `0600` secrets file.

Kaggle is available from setup, `isaacli kaggle`, `/kaggle`, and the `/model` provider menu. The shared flow stores any number of Kaggle accounts, shows each account's remaining quota and lets the user select one manually. Prepared assets are private and belong to the selected account. When they do not exist, the flow announces the measured startup cost, offers CPU-only preparation, and keeps the self-contained launch path available. Every GPU push requires confirmation at that moment, and it asks how long that kernel may live: the kernel holds that ceiling itself and Kaggle holds the same figure, so a window that goes away cannot leave one running for hours. It also ends itself after five minutes without a sign of life from the window that launched it. Kaggle is a third-party service whose sessions can stop, and its terms were not designed around notebooks as persistent API servers.

A Kaggle session costs quota by wall clock for as long as its kernel is alive, so isaacli takes responsibility for the whole life of the one it started: closing the program ends that kernel, opening it again reuses the endpoint if it is still serving, which costs nothing, and pushing another one is never automatic because that is where the spending is. `isaacli kaggle --stop` ends a session from outside, and `uninstall --purge --kaggle` lists what is left on the account and asks before deleting any of it. Two isaacli windows can share one kernel: the last one to close is the one that ends it. What you chose last time, the account and the model and the exact precision, is remembered and offered back as one keypress, and changing it is the other option on that same screen.
...
- Workspace-root `AGENTS.md` project instructions, bounded and subordinate to the built-in safety contract.
...
- Ollama is the recommended engine because it is a single installer with a model catalog and because it ships the CUDA backend, not because it is faster. The two were measured against each other on this project's hardware on 2026-08-21, same GGUF, same context, same prompt, orders swapped: Ollama 39.36 tok/s against llama-server 37.34, a 5.4% gap, with llama-server's own spread between two rounds at 2.81 tok/s. The difference is inside the machine's noise, so no speed claim is made for either, in the interface or here. Per-model throughput on this card is in [Measurements](docs/benchmarks/README.md).
